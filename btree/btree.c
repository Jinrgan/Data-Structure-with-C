...
btree_pnode create_total_btree(int i, int nmem)
{
	btree_pnode new;
	//创建根结点
	new = (btree_pnode)malloc(sizeof(btree_node));
	...
	if(2*i <= nmem)
		new->lchild = create_total_btree(2*i, nmem);
	else
		new->lchild = NULL;
	
	if(2*i+1 <= nmem)
		new->rchild = create_total_btree(2*i+1, nmem);
	else
		new->rchild = NULL;
...
//中序非递归遍历
void unmid_order(btree_pnode t)
{
	linklist top;//top为指向栈顶结点的指针
